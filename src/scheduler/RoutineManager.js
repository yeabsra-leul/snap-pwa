...
import { Routine, Repeat, TimeInterval, Interval} from "./Routine";
...
class RoutineManager{
    ...
    allocateRoutine = function(day) {
        ...
        for(let routine of this._routines){
            if( day.date.getDay() == routine.repeat || routine.repeat == Repeat.AllDAY){
    ...
    removeRoutine = function(routine){
        this._routines = this._routines.filter( item =>
            routine.id != item.id
        )
    }
...
// // test 
routineManager.addRoutine(new Routine(0, 1300, 1350, "Kill the dog", Repeat.FRI))
